...
def main():
    ...
    test_ds = TwoImageFolder(ctor=ImageFolder, root="level_05/test_data", transform=nn.Identity())
    ...
    for epoch in range(100):
        ...
        with torch.no_grad():
            ...
            for x, _, _ in tqdm(test_dl):
                ...
                y_hats.append(torch.concat(cur_y_hats, dim=1))
            y_hats = torch.concat(y_hats)
            y_hats = y_hats.round().long()

            # write
            lines = [",".join(map(str, y_hat.tolist())) for y_hat in y_hats]
