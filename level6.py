...
def y_hat_to_coord(y_hat, blur):
    with torch.no_grad():
        ...
        blurred += torch.randn_like(blurred) * 0.001
...
def main():
    ...
    test_ds = TwoImageFolder(ctor=ImageFolder, root="level_06/test_data", transform=nn.Identity())
    ...
    for epoch in range(100):
        ...
        padding = 12
        fence_color = torch.tensor([64, 40, 36]).to(device) / 255
        #model.eval()
        with torch.no_grad():
            counts = []
            for x, _, _ in tqdm(test_dl):
                ...
                for i in range(5):
                    ...
                    for j in range(len(y_hat)):
                        a, b = coords_hat[j]
                        y_hat[j, b - padding:b+padding, a - padding: a + padding] = -10
                cur_y_hats = torch.concat(cur_y_hats, dim=1)
                is_fence = (x == fence_color[None, :, None, None]).sum(dim=1) == 3
            ...
            # write
            lines = [str(count) for count in counts]
